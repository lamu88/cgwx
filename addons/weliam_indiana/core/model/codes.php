...
class Welian_Indiana_Codes{
	...
	public function code($openid='',$tid = '',$uniacid=0,$machine_time = ''){
		...
		foreach ($myCart as $key => $value) {
			...
			m('log')->WL_log('codes','剩余消费数',$left_money,$uniacid);
			//判定消费金额
			$data_period = array(
				':uniacid' => $_W['uniacid'],
	...
	public function createtime_winer($periodid = '',$period_number = ''){
		...
		self::get_winner($period_number, $periodid, $wincode , $period['id']);		//执行中奖人信息

	}
	/******************获取中奖人信息****************/
	public function get_winner($period_number = '', $periodid = '' , $wincode = '', $periodid = ''){
		global $_W;
		//更新完毕，计算获奖信息
