...
$fid=intval($_GPC['fid']);
if($fid>0){
//     $job['title']=pdo_fetchcolumn("select title from ".tablename('enjoy_city_firm')." where uniacid=".$uniacid."
//         and id=".$fid."");
...
if($op=='delete'){
	//$id = intval($_GPC['id']);
 	$id=$_GPC['id'];
 	//删除job表
 	$res=pdo_delete('enjoy_city_job', array('id' => $id,'uniacid'=>$uniacid));
...
pdo_update('enjoy_city_job',array('ischeck'=>$ischeck),array('uniacid'=>$uniacid,'id'=>$id));
...
if(checksubmit('submit')){
	if (!empty($_GPC['hot'])) {
		foreach ($_GPC['hot'] as $id => $hot) {
			pdo_update('enjoy_city_job', array('hot' => $hot), array('id' => $id));
		}
		message('排序更新成功！', $this -> createWebUrl('job', array('op' => 'display')), 'success');
	}
}
...
//实际参加人数
$countadd=pdo_fetchcolumn("select a.*,b.title from ".tablename('enjoy_city_job')." as a left join
...
$countcheck=pdo_fetchcolumn("select count(*) from ".tablename('enjoy_city_job')." where uniacid=".$uniacid." and ischeck=1");
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('enjoy_city_job', $data, array('id' => $id));
	...
	message($message, $this->createWebUrl('job', array('op' => 'display')), 'success');
	}
	//修改
//	$job = pdo_fetch("SELECT * FROM " . tablename('enjoy_city_job') . " WHERE id = '$id' and uniacid = '{$_W['uniacid']}'");
