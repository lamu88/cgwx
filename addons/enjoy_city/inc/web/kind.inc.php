...
if ($operation == 'display') {
	if (!empty($_GPC['hot'])) {
		foreach ($_GPC['hot'] as $id => $hot) {
			pdo_update('enjoy_city_kind', array('hot' => $hot), array('id' => $id));
		}
		message('分类排序更新成功！', $this -> createWebUrl('kind', array('op' => 'display')), 'success');
	}
	$children = array();
	...
	if (checksubmit('submit')) {
		...
		$data = array('uniacid' => $_W['uniacid'], 'name' => $_GPC['catename'], 'enabled' => intval($_GPC['enabled']), 'hot' => intval($_GPC['hot']),'parentid' => intval($parentid), 'thumb' => $_GPC['thumb'],'wurl' => trim($_GPC['wurl']));
		if (!empty($id)) {
			unset($data['parentid']);
			pdo_update('enjoy_city_kind', $data, array('id' => $id));
			load() -> func('file');
		...
		message('更新分类成功！', $this -> createWebUrl('kind', array('op' => 'display')), 'success');
	}
	include $this -> template('kind');
	...
	if (empty($kind)) {
		message('抱歉，分类不存在或是已经被删除！', $this -> createWebUrl('kind', array('op' => 'display')), 'error');
	}
	pdo_delete('enjoy_city_kind', array('id' => $id, 'parentid' => $id), 'OR');
	message('分类删除成功！', $this -> createWebUrl('kind', array('op' => 'display')), 'success');
    ...
    message('初始化分类成功！', $this -> createWebUrl('kind', array('op' => 'display')), 'success');
