...
if($op=='delete'){
	//$id = intval($_GPC['id']);
 	$id=$_GPC['id'];
 	//删除contact表
 	$res=pdo_delete('enjoy_city_contact', array('id' => $id,'uniacid'=>$uniacid));
...
pdo_update('enjoy_city_contact',array('ischeck'=>$ischeck),array('uniacid'=>$uniacid,'uid'=>$uid));
...
if(checksubmit('submit')){
	if (!empty($_GPC['hot'])) {
		foreach ($_GPC['hot'] as $id => $hot) {
			pdo_update('enjoy_city_contact', array('hot' => $hot), array('id' => $id));
		}
		message('排序更新成功！', $this -> createWebUrl('contact', array('op' => 'display')), 'success');
	}
}
...
$countcheck=pdo_fetchcolumn("select count(*) from ".tablename('enjoy_city_contact')." where uniacid=".$uniacid." and ischeck=1");
	...
	if (checksubmit('submit')) {
		...
		if (!empty($id)) {
			pdo_update('enjoy_city_contact', $data, array('id' => $id));
	...
	message($message, $this->createWebUrl('contact', array('op' => 'display')), 'success');
	}
	//修改
