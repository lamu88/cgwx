...
if (!empty($cfg['mchid'])) {
    ...
    $wpayconfig    = new WxPayConf_micropay($appid, $mchid, $apikey, $cert->certpath, $cert->keypath);
    $redpk         = new Entpayment_redpack($wpayconfig);
    $mch_billno    = $wpayconfig->mchid . date('Ymd', time()) . time();
    $redpk->setParameter("mch_billno", $mch_billno);
    $redpk->setParameter("re_openid", $member['openid']);
    $redpk->setParameter("send_name", $_W['account']['name']);
    $redpk->setParameter("total_amount", $dtotal_amount);
    ...
    try {
        ...
        pdo_insert('wdl_jifenbao_tixianlog', array(
            'uniacid' => $_W["uniacid"],
            "dwnick" => $member['nickname'],
