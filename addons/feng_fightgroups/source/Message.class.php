...
class Message {
	 ...
	 *
	 */
	public function daipay_success($openid, $price,$name,$orderno, $goodsname,$time, $message,$the, $url) {
		global $_W;
		load() -> func('communication');
		load() -> model('account');
		$m_daipay = $the -> module['config']['m_daipay'];
		//支付成功模板消息提醒
		$content = "您已成功付款";
		$postdata  = array(
					"first"=>array( "value"=> "亲，您的朋友代帮你支付了一笔订单","color"=>"#173177"),
					"keyword1"=>array('value' => $orderno, "color" => "#4a5077"),
					"keyword2"=>array('value' => $name, "color" => "#4a5077"),
					"keyword3"=>array('value' => $price."元[".$goodsname."]", "color" => "#4a5077"),
					"keyword4"=>array('value' => $time, "color" => "#4a5077"),
					"keyword5"=>array('value' => $message, "color" => "#4a5077"),
					"remark"=>array("value"=>'点击查看详情', "color" => "#4a5077"),
				);
		self::sendTplNotice($openid, $m_daipay, $postdata,$url);
	}
	/**
	 *
	 * 模板消息
