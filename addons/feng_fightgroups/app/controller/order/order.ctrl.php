...
if($op =='tip'){
	$list = pdo_fetch("SELECT * FROM ".tablename('tg_order')." WHERE uniacid = {$_W['uniacid']} AND status in(1,2,3,4) AND openid<>'' and mobile<>'虚拟' order by rand()"); 
	$sql = 'SELECT * FROM '.tablename('tg_member').' WHERE openid=:openid and uniacid=:uniacid';
	$paramse = array(':openid'=>$list['openid'], ':uniacid'=>$_W['uniacid']);
	$members = pdo_fetch($sql, $paramse);		
	$list['nickname'] = mb_substr($members['nickname'], 0,3,'utf-8');	
	$list['avatar'] = $members['avatar'];	
	$citylist = pdo_fetch("select * from ".tablename('tg_address')." where uniacid = {$_W['uniacid']} and openid='{$list['openid']}'");
