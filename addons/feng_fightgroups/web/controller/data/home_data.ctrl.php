<?php
	...
	$goods = goods_get_list(array('merchantid' => MERCHANTID));
	$orders = pdo_fetchall("select id from".tablename('tg_order')."where uniacid={$_W['uniacid']} and status in(0,1,2,3,4,6,7) ".TG_MERCHANTID."");
...
//	wl_debug($orders);
	...
	$yesterdayorders1 =  pdo_fetchall("select id from".tablename('tg_order')."where uniacid={$_W['uniacid']} and ptime>'{$ystd}' and ptime<'{$today}' and status in('1,2,3,4,6,7') ".TG_MERCHANTID."");
	$yorders1 = count($yesterdayorders1);
	$yesterdayorders2 =  pdo_fetchall("select id from".tablename('tg_order')."where uniacid={$_W['uniacid']} and successtime >'{$ystd}' and successtime<'{$today}' and status in('1,2,3,4,6,7') ".TG_MERCHANTID."");
	$yorders2 = count($yesterdayorders2);
	$yesterdayorders3 =  pdo_fetchall("select id from".tablename('tg_order')."where uniacid={$_W['uniacid']} and sendtime>'{$ystd}' and sendtime<'{$today}' and status in('1,2,3,4,6,7') ".TG_MERCHANTID."");
