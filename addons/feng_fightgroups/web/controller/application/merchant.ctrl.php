<?php
	...
	$ops = array('display','edit', 'delete', 'account_display','account','record','data','permissions','search','jiesuan');
	$op_names = array('商家列表','编辑/添加商家', '删除', '商家中心管理','商家结算中心','结算记录','订单统计','权限管理','查找粉丝','结算');
	...
	if($op == 'data'){
		...
		$issettlement = $_GPC['issettlement'];
		$status = $_GPC['status'];
		...
		if ($issettlement=='no') {
			$condition .= " AND  issettlement is null and status in(3,4) ";
		}
		if ($issettlement=='yes') {
			$condition .= " AND  issettlement = 1 ";
		}
		if ($status != '') {
		...
		$sql = "select  * from " . tablename('tg_order') . " where $condition and mobile <> '虚拟' and merchantid = {$id} ORDER BY createtime DESC " . "LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
		...
		foreach ($list as $key => $value) {
			...
			$list[$key]['settlementprice'] = pdo_fetch("select * from" . tablename('tg_merchant_record') . "where orderno = '{$value['orderno']}' and uniacid={$_W['uniacid']}");
		}
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE $condition and merchantid={$id} and mobile<>'虚拟'", $paras);
		$pager = pagination($total, $pindex, $psize);
		$jiesuan_no = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and mobile<>'虚拟' and issettlement is null and status in(3,4) and merchantid={$id}");
		$jiesuan_yes = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and mobile<>'虚拟' and issettlement = 1 and merchantid={$id}");
		$all = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and mobile<>'虚拟' and merchantid={$id}");
		$status0 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=0 and mobile<>'虚拟'");
		$status1 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=1 and mobile<>'虚拟'");
		$status2 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=2 and mobile<>'虚拟'");
		$status3 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=3 and mobile<>'虚拟'");
		$status4 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=4 and mobile<>'虚拟'");
		$status5 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=5 and mobile<>'虚拟'");
		$status6 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=6 and mobile<>'虚拟'");
		$status7 = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid={$id}  and status=7 and mobile<>'虚拟'");
		include wl_template('application/merchant/account');
	}
	if($op == 'permissions'){
		$id = intval($_GPC['id']);
	...
	if($op == 'jiesuan'){
		...
		if(!empty($_GPC['id'])){
			...
			$get_money = $money;
			if(!empty($merchant['percent'])){
				$get_money = (1-$merchant['percent']*0.01)*$money;
			}
			$get_money = currency_format($get_money);
			if(is_numeric($get_money)){
				if($get_money<1){
					$message = "到账金额需要大于1元!";
					die(json_encode(array('errno'=>1,'message'=>$message)));
				}

				$result = finance($merchant['openid'], 1, $get_money * 100, '', '商家提现');
				if (is_error($result)) {
					$message = '微信钱包提现失败: ' . $result['message'];
					die(json_encode(array('errno'=>1,'message'=>$message)));
				}
				if($result){
					$res = pdo_update('tg_order', array('issettlement'=>1),array('id' => $id));
					if($res){
						merchant_update_no_money(0-$money, $merchantid);
						pdo_insert("tg_merchant_record",array('orderno'=>$data['orderno'],'percent'=>$merchant['percent'],'commission'=>$money-$get_money,'merchantid'=>$merchantid,'money'=>$money,'get_money'=>$get_money,'uid'=>$_W['uid'],'createtime'=>TIMESTAMP,'uniacid'=>$_W['uniacid']));
						$message = "结算成功！";
			...
			$jiesuan_num = count($order_ids_arr);
			$allmoney = 0;
			for($i=0;$i<$jiesuan_num;$i++){
				$data = pdo_fetch("SELECT price,issettlement FROM " . tablename('tg_order') . " WHERE id = :id and uniacid={$_W['uniacid']}", array(':id' => $order_ids_arr[$i]));
				if($data['issettlement']==1){
					$message = "该订单已结算!";
					continue;
				}
			...
			if(is_numeric($allmoney)){
				$result = finance($merchant['openid'], 1, $allmoney * 100, '', '商家提现');
				if (is_error($result)) {
					$message = '微信钱包提现失败: ' . $result['message'];
					message($message,web_url('application/merchant/data',array('id'=>$merchantid)),'errno');exit;
				}else{
					for($i=0;$i<$jiesuan_num;$i++){
						$res = pdo_update('tg_order', array('issettlement'=>1),array('id' => $order_ids_arr[$i]));
						$data = pdo_fetch("SELECT price,orderno FROM " . tablename('tg_order') . " WHERE id = :id and uniacid={$_W['uniacid']}", array(':id' => $order_ids_arr[$i]));
						pdo_insert("tg_merchant_record",array('orderno'=>$data['orderno'],'percent'=>$merchant['percent'],'commission'=>$data['price']*$merchant['percent']*0.01,'merchantid'=>$merchantid,'money'=>$data['price'],'get_money'=>$data['price']-$data['price']*$merchant['percent']*0.01,'uid'=>$_W['uid'],'createtime'=>TIMESTAMP,'uniacid'=>$_W['uniacid']));
					}
					merchant_update_no_money(0-$allmoney, $merchantid);
					$message = "结算成功！";
					message($message,web_url('application/merchant/data',array('id'=>$merchantid)),'success');exit;
				}
			}else{
				$message = '结算金额非法!';
				message($message,web_url('application/merchant/data',array('id'=>$merchantid)),'success');exit;
