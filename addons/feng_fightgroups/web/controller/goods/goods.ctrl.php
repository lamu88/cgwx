...
if ($op == 'display') {
	...
	$size = 10;
	$condition = " WHERE 1 and uniacid = {$_W['uniacid']} ".TG_MERCHANTID." ";
...
//	wl_debug($goodses);
	...
	$merchants = pdo_fetchall("SELECT * FROM " . tablename('tg_merchant') . " WHERE uniacid = '{$_W['uniacid']}' ".TG_ID."  ORDER BY id DESC");
	$muban_array = array();
	$dispatch_list = pdo_fetchall("select * from".tablename('tg_delivery_template')."where uniacid={$_W['uniacid']} and status=2");
	...
	if (!empty($id)) {
		...
		//获取当前图集
		$listt = pdo_fetchall("SELECT * FROM" . tablename('tg_goods_atlas') . "WHERE g_id = '{$id}' order by id desc");
		$piclist = array();
		if (is_array($listt)) {
			foreach ($listt as $p) {
				$piclist[] = $p['thumb'];
	...
	if (checksubmit()) {
		...
		if (empty($goods)) {
			...
			$oplogdata = serialize($data['sitepic']);
			oplog('admin', "添加商品", web_url('goods/goods/post'), $oplogdata);
			if ($images) {
				foreach ($data['sitepic'] as $key => $value) {
	...
	foreach($goods_ids as $key =>$id){
		if($funcop=='on_shelves'){
			if(goods_update_by_params(array('isshow'=>1), array('id'=>$id))){
				/*抽奖团*/
				pdo_update('tg_lottery',array('status'=>1),array('goodsid'=>$id));
			}else{
			}
		}
		if($funcop=='off_shelves'){
			if(goods_update_by_params(array('isshow'=>2), array('id'=>$id))){
				/*抽奖团*/
				pdo_update('tg_lottery',array('status'=>2),array('goodsid'=>$id));
			}else{
			}
		...
		if($funcop=='remove'){
			...
			if(goods_delete($id)){
				/*抽奖团*/
				pdo_delete('tg_lottery',array('goodsid' => $id));
